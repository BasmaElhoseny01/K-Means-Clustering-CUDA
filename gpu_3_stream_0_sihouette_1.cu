...
*/
__global__ void compute_shetollute_score(float *d_data_points, int *d_cluster_assignment, float *d_centroids, int N, int D, int K, float *d_shilloute_scores_sum)
{
    ...
    // Compute the sum of shilloute scores
    // Only the first thread in the block will compute the sum
    if (block_tid == 0)
    {
        float sum = 0;
        ...
        // Store the sum in the global memory in the corresponding block index

        atomicAdd(&d_shilloute_scores_sum[0], sum);
        // d_shilloute_scores_sum[blockIdx.x] = sum;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    float *shilloute_scores = (float *)malloc(sizeof(float)); // Array to store the size of each cluster
    ...
    cudaMalloc(&d_shilloute_scores, sizeof(float)); // Array to store the size of each cluster
    ...
    cudaMemcpy(shilloute_scores, d_shilloute_scores, sizeof(float), cudaMemcpyDeviceToHost); // Take only the first element

    // Compute the average shilloute score
    float shetollute_score = shilloute_scores[0];
...
// ./out_gpu_3_stream_0_sihouette_1 .\tests\image_3.png 5
// D:\Parallel-Computing-Project>nvprof -o ./profiles/out_gpu_3_stream_0_sihouette_1.nvprof ./out_gpu_3_stream_0_sihouette_1 ./tests/image_3.png 5
