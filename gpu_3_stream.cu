...
__global__ void assign_data_points_to_centroids(int N_segment, int D, int K, float *d_data_points, float *d_centroids, int *d_cluster_assignment)
{
    ...
    //     float dist = distance(d_data_points + block_tid * D, sh_centroids + i * D, D);
    ...
    // d_cluster_assignment[block_tid] = min_centroid;
    // printf("%d ", min_centroid);
...
int main(int argc, char *argv[])
{
    ...
    while (iteration < MAX_ITERATIONS)
    {
        ...
        // Stream the Segments
        int num_segments = num_streams;
        int segment_size = (N + num_segments - 1) / num_segments; // ceil(N/num_segments)

        for (int s = 0; s < num_segments; s++)
        {
            // Getting Segement Bounds
            int start = s * segment_size * D;
            // printf("start %d\n", start);
            int end = start + segment_size * D; // Excluded
            if (end > N * D)
            {
                end = N * D;
            }

            int N_segment = end - start; // same as segment_size except for case that end is out of boundary

            int num_blocks = (N_segment + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK; // ceil(N_segment/THREADS_PER_BLOCK)
            // int num_blocks = (segment_size + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK; // ceil(N/THREADS_PER_BLOCK)

            // call the kernel [assign_data_points_to_centroids]
            assign_data_points_to_centroids<<<num_blocks, THREADS_PER_BLOCK, K * D * sizeof(float), stream[s]>>>(N_segment, D, K, d_image + start, d_centroids, d_cluster_assignment + start);
            ...
            // Kernel(2)
            // update_cluster_centroids<<<num_blocks, THREADS_PER_BLOCK, 0, stream[s]>>>(N, D, d_image + start, d_cluster_assignment + start, d_centroids, d_cluster_sizes, K);
        }
...
// nvcc -o out_gpu_3_stream  ./gpu_3_stream.cu
// ./out_gpu_3_stream .\tests\image_3.png 5